...
export async function GET(req: Request) {
  ...
  if (weddingSlug) resolvedWeddingId = String(await resolveWeddingIdBySlug(weddingSlug));
  ...
  try {
    ...
    const items = await ChecklistItemModel.find({ weddingId: resolvedWeddingId })
      .sort({ section: 1, sortOrder: 1, createdAt: 1 })
      .lean();
    return jsonOk({ items });
  } catch {
    return jsonError("Failed to load checklist", 500);
  }
}

export async function POST(req: Request) {
  try {
    const parsed = ChecklistCreateSchema.safeParse(await req.json());
    ...
    await connectToDatabase();
    const item = await ChecklistItemModel.create(parsed.data);
    return jsonCreated({ item });
  } catch {
    return jsonError("Failed to create checklist item", 500);
  }
}
