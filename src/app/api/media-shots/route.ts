...
import { MediaShotCreateSchema } from "@/lib/validators";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const items = await MediaShotModel.find({ weddingId: resolvedWeddingId }).sort({ category: 1, createdAt: 1 }).lean();
    return jsonOk({ items });
  } catch {
    return jsonError("Failed to load media shots", 500);
  }
}

export async function POST(req: Request) {
  try {
    const parsed = MediaShotCreateSchema.safeParse(await req.json());
    ...
    await connectToDatabase();
    const item = await MediaShotModel.create(parsed.data);
    return jsonCreated({ item });
  } catch {
    return jsonError("Failed to create media shot", 500);
  }
}
